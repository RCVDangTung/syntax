...
//                $replace = str_replace('[BASE_URL]', 'http://localhost/manager-autosale', $replace);
                ...
                $replace = str_replace('[DATE_TIME_STUDY]', date('H:i d/m/Y',strtotime($item->trial_time)), $replace);
    ...
    public function send_email_sms_auto() {
    	...
    	$list_contact = $this->m_contact_submit->get_list_contact_check(array(
           // 'm.contact_id' => '20151212000683ASTL',
            // 'c.level_care' => 'A3',
            // 'cd.type' => '5',
    		'm.status' => 1
    		));
        	...
        	foreach ($list_contact as $key => $value) {
                ...
                if ($value->auto_flag == 0 && $value->level_care == 'A3' && ( (time() - strtotime($value->time_created)) >= 60)) { //Gui email counter-service
                	...
                	if ($value->type == 5 && $value->flag_email_counter_service == 0) {
                		...
                		$care_data_couterse["id"] = $care_id_couterse;
                		...
                		// Gửi SMS Counter Service
                		$care_data_couterse = array();
                		$care_data_couterse["contact_id"] = $value->contact_id;
                		$care_data_couterse["content_care"] = $arr_sms_temp['SMS_COUNTER_SERVICE']->content;
                		$care_data_couterse["time_created"] = time();
                		$care_data_couterse["care_type_code"] = 'SMS_COUNTER_SERVICE';
                		$care_data_couterse["note"] = $arr_sms_temp['SMS_COUNTER_SERVICE']->subject;
                		$care_data["status"] = "success";
                		$care_id_couterse = $this->m_care_history->add($care_data_couterse);
                		$care_data_couterse["id"] = $care_id_couterse;
                		$this->m_care_history_month->add($care_data_couterse);

                		$sms_data_couterse = array();
                		$sms_data_couterse["id"] = $care_id;
                		$sms_data_couterse["status_send"] = 1;
                		$sms_data_couterse["phone"] = $value->phone;
                		$sms_data_couterse["content"] = $arr_sms_temp['SMS_COUNTER_SERVICE']->content;
                		$sms_data_couterse["template_id"] = $arr_sms_temp['SMS_COUNTER_SERVICE']->id;
                		$sms_data_couterse["subject"] = $arr_sms_temp['SMS_COUNTER_SERVICE']->subject;
                		$sms_insert_id_couterse = $this->m_sms_history->add($sms_data_couterse);

                		//Cap nhat flag_email_counter_service = 1
                		if($sms_insert_id_couterse && $email_insert_id_couterse){
                			$this->m_contact_submit->update($value->id, array('flag_email_counter_service' => 1));
                		}
                		
                	}
                    // gửi email SMS Bank transfer
                    if($value->type == 0 && $value->flag_email_atm == 0){
                        $care_data_atm = array();
                        $care_data_atm["contact_id"] = $value->contact_id;
                        $care_data_atm["content_care"] = '';
                        $care_data_atm["time_created"] = time();
                        $care_data_atm["care_type_code"] = 'EMAIL';
                        $care_data_atm["note"] = $arr_email_temp['EM_ATM']->subject;
                        $care_data_atm["status"] = "success";
                        $care_id_atm = $this->m_care_history->add($care_data_atm);
                        $care_data_atm["id"] = $care_id_atm;
                        $this->m_care_history_month->add($care_data_atm);

                        $email_data_atm = array();
                        $email_data_atm["id"] = $care_id_couterse;
                        $email_data_atm["status_send"] = 1;
                        $email_data_atm["email_to"] = $value->email;
                        $email_data_atm["template_id"] = $arr_email_temp['EM_ATM']->id;
                        $email_data_atm["subject"] = $arr_email_temp['EM_ATM']->subject;
                        $email_insert_id_atm = $this->m_email_history->add($email_data_atm);

                        //SMS
//                     $care_data = array();
                        $care_data_atm["contact_id"] = $value->contact_id;
                        $care_data_atm["content_care"] = $arr_sms_temp['SMS_BANK_TRANSFER']->content;
                        $care_data_atm["time_created"] = time();
                        $care_data_atm["care_type_code"] = 'SMS';
                        $care_data_atm["note"] = $arr_sms_temp['SMS_BANK_TRANSFER']->subject;
                        $care_data_atm["status"] = "success";
                        $care_id_atm = $this->m_care_history->add($care_data_atm);
                        $care_data_atm["id"] = $care_id_atm;
                        $this->m_care_history_month->add($care_data_atm);
                        ...
                        $sms_data_atm = array();
                        $sms_data_atm["id"] = $care_id_atm;
                        $sms_data_atm["status_send"] = 1;
                        $sms_data_atm["phone"] = $value->phone;
                        $sms_data_atm["content"] = $arr_sms_temp['SMS_BANK_TRANSFER']->content;
                        $sms_data_atm["template_id"] = $arr_sms_temp['SMS_BANK_TRANSFER']->id;
                        $sms_data_atm["subject"] = $arr_sms_temp['SMS_BANK_TRANSFER']->subject;
                        $sms_insert_id_atm = $this->m_sms_history->add($sms_data_atm);

                        if($sms_insert_id_atm && $email_insert_id_atm){
                            $this->m_contact_submit->update($value->id, array('flag_email_atm' => 1));
                        }
                    }
