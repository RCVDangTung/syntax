...
//    var $server = 'http://lmsautosale.local.topicanative.edu.vn/webservice/rest/server.php';
    var $server = 'http://autosale.lms.topicanative.edu.vn/webservice/rest/server.php';
    ...
    function send_email_queue() {
        ...
        if (count($list_email_send)) {
            foreach ($list_email_send as $item) {
                ...
                $replace = str_replace('[VOUCHER_URL]', "http://manager.topicanative.co.th/get_voucher" . '?id=' . $item->contact_id, $replace);
                $replace = str_replace('[VOUCHER_URL_TENUP]', "http://manager.topicanative.co.th/get_voucher_tenup" . '?id=' . $item->contact_id, $replace);
                $replace = str_replace('[VOUCHER_URL_NORMAL]', "http://manager.topicanative.co.th/get_voucher_normal" . '?id=' . $item->contact_id, $replace);


                $base_url = preg_replace("{/$}", "", base_url());
                $replace = str_replace('[LINK_TECH_NEWBIE]', 'http://autosale.lms.topicanative.edu.vn/local/tpebbb/process.php?roomType=TECHSUPPORTNEWBIE&from=LMS&classType=OTHER', $replace);
                $replace = str_replace('[BASE_URL]', 'http://manager.topicanative.co.th', $replace);
//                $replace = str_replace('[BASE_URL]', 'http://localhost/manager-autosale', $replace);
                $replace = str_replace('[LINK_LMS]', 'http://topi.ca/hoc-thu-at', $replace);
                $replace = str_replace('[VIDEO_GIOI_THIEU]', 'https://www.youtube.com/watch?v=5rCuHUPw_fM&feature=youtu.be', $replace);
                $replace = str_replace('[VIDEO_HUONG_DAN]', 'https://www.youtube.com/watch?v=o8qeft4UHEE&feature=youtu.be', $replace);
                ...
                $email = $this->m_config_email->send_email(NULL, NULL, 'topica.native@topica.edu.vn', 'TOPICA NATIVE', $item->email_to, $item->content, $item->subject, "TOPICA NATIVE", NULL, NULL);
    ...
    public function send_email_sms_auto() {
        ...
        $list_contact = $this->m_contact_submit->get_list_contact_check(array(
            // 'm.contact_id' => '20151125000002ASTL',
            // 'm.auto_flag <=' => 7,
            // 'm.date >' => '2015-09-24',
            'm.status' => 1
            ));
            ...
            if($item->classnumber){
                for($i = 1; $i <= $item->classnumber; $i++){
                    $temp = array(
                        ...
                        'assistant_id' => 0,
                        'teacher_backup' => '',
                        'calendar_code' => date('Ymd', strtotime($item->date)) . '-' . 'AT1000' . '-' . $item->id . $i,
                        'level_class' => 'basic',
                        'teacher_type' => 'AM',
                        'address_teach' => '',
                        'type_class' => 'LS',
